...
public interface PayInstanceFactory extends Factory<PayInstance> {
    ...
    static PayInstance create(int payType){
        String className;
        switch (payType){
            case Constant.ORDER_PAY_TYPE_ALIPAY: className = PayInstanceAlipayFactory.class.getName();break;
            case Constant.ORDER_PAY_TYPE_WECHAT: className = PayInstanceWechatFactory.class.getName();break;
