...
public class DictResource extends DataResource<DictService, Dict> {
	...
	@Timed
	public ResponseEntity save(Dict dict) throws URISyntaxException {
		log.debug("REST request to save Dict : {}", dict);
		// Lowercase the dict login before comparing with database
		if (!checkByProperty(Reflections.createObj(Dict.class, Lists.newArrayList(Dict.F_ID, Dict.F_CODE), dict.getId(),
				dict.getName()))) {
